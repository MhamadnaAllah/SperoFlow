...
public sealed class RsaServiceTokenFactory : IServiceTokenFactory
{
        ...
        IReadOnlyDictionary<string, string>? additionalClaims = null)
    {
        ...
        var claims = new List<Claim>
        {
            ...
            new(JwtRegisteredClaimNames.Iat, EpochTime.GetIntDate(now).ToString(System.Globalization.CultureInfo.InvariantCulture), ClaimValueTypes.Integer64),
            new("scope", scope),
        };
        if (userId.HasValue)
        {
...
public sealed class RsaServiceTokenValidator : IServiceTokenValidator
{
    ...
    private static RsaSecurityKey LoadValidationKey(ServiceJwtOptions options)
    {
