...
logger = logging.getLogger("speroflow.dataset_worker")
...
def _process_dataset_job(settings: Settings, job: dict[str, Any]) -> DatasetJobOutcome:
    required = ("jobId", "datasetId", "sourceFileId", "ownerId", "datasetName", "objectKey", "fileName", "contentType", "sizeBytes", "sha256")
    missing = [key for key in required if not job.get(key)]
    if missing:
        raise DatasetIngestionError("Dataset job payload is missing: " + ", ".join(missing))

    with tempfile.TemporaryDirectory(prefix="speroflow-dataset-") as temp_dir:
        ...
        dataset = DatasetGraphRecord(str(job["datasetId"]), str(job["ownerId"]), str(job["datasetName"]))
        source = SourceFileGraphRecord(
            str(job["sourceFileId"]),
            str(job["datasetId"]),
            str(job["ownerId"]),
            str(job["fileName"]),
            str(job["objectKey"]),
